...
//CONSTRUCTORS
    game_elements::boat::boat(const coordinates& begin, const coordinates& end){
        ...
        corazza_ = dimension_; 
        //std::cout<<std::endl<<"BEGIN X: "<<coord_begin_.get_x()<<" END X:"<<coord_end_.get_x()<<std::endl;
        vertical_ = ((coord_begin_.get_x() - coord_end_.get_x()) == 0);
        //std::cout<<std::boolalpha<<vertical_<<std::endl;
    }
...
//HELPER FUNCTIONS
    ...
    bool game_elements::check_coordinates(const coordinates& begin, const coordinates& end){
        ...
        //std::cout<<begin.get_x()<<" "<<begin.get_y()<<std::endl;
        return false;
    }